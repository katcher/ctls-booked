...
$conf['settings']['host'] = 'int-con-dc-3.concordia.ca'; // comma separated list of ldap servers such as mydomain1,localhost
...
$conf['settings']['binddn'] = 'CN=iits_portal,OU=Roles,DC=concordia,DC=ca';	// The distinguished name to bind as (username). If you don't supply this, an anonymous bind will be established.
$conf['settings']['bindpw'] = 'Esp=mc2';	// Password for the binddn. If the credentials are wrong, the bind will fail server-side and an anonymous bind will be established instead. An empty bindpw string requests an unauthenticated bind.
$conf['settings']['basedn'] = 'OU=People,DC=concordia,DC=ca';	// LDAP base name (eg. dc=example,dc=com)
